class Solution {
    ...
    public int wiggleMaxLength(int[] nums) {
        ...
        for (int i = 1; i < n; ++i) {
            ...
            // }
            if (nums[i] > nums[i - 1]) {
                wiggleSubsequence[i][1] = wiggleSubsequence[i - 1][0] + 1;
                wiggleSubsequence[i][0] = wiggleSubsequence[i - 1][0];
            } else if (nums[i] < nums[i - 1]) {
                wiggleSubsequence[i][0] = wiggleSubsequence[i - 1][1] + 1;
                wiggleSubsequence[i][1] = wiggleSubsequence[i - 1][1];
            } else {
                wiggleSubsequence[i][1] = wiggleSubsequence[i - 1][1];
                wiggleSubsequence[i][0] = wiggleSubsequence[i - 1][0];
            }
